...
export async function PUT(req: Request) {
  try {
    ...
    for (const item of items) {
      if (item.id && item.displayOrder !== undefined) {
        await db
          ...
          .where(eq(problemStatements.id, item.id));
      }
    }
    ...
    console.error('Failed to reorder problem statements:', err);
    return NextResponse.json(
      { error: { code: 'INTERNAL_ERROR', message: err.message || 'Failed to reorder problem statements' } },
      { status: 500 }
    );
  }
}
