...
export default function ContactQueriesPage() {
    ...
    async function loadLiveQueries() {
      try {
        ...
        if (res.ok) {
          ...
          const local = localStorage.getItem("asg_queries");
          let baseItems = INITIAL;
          if (local) {
            try {
              baseItems = JSON.parse(local) as QueryItem[];
            } catch { /* ignore */ }
          }
          
          const dbIds = new Set(dbQueries.map((q: any) => q.id));
          const nonDbQueries = baseItems.filter(q => !dbIds.has(q.id));
          setQueries([...nonDbQueries, ...dbQueries]);
        } else {
          const local = localStorage.getItem("asg_queries");
          if (local) setQueries(JSON.parse(local));
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (queries !== INITIAL) {
      localStorage.setItem("asg_queries", JSON.stringify(queries));
    }
  }, [queries]);
  ...
  const resolveQuery = (id: string | number) => {
