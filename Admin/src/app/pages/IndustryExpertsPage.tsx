...
// Load problem statements from AAL localStorage or fallback
...
export default function IndustryExpertsPage() {
  const [experts, setExperts, undo, redo, canUndo, canRedo] = useUndoRedoState<Expert[]>(() => {
    ...
    localStorage.setItem("asg_experts", JSON.stringify(INITIAL));
    return INITIAL;
  });
      ...
      {modal.open && (modal.mode === "add" || modal.mode === "edit") && (
        <Modal isOpen={true} onClose={close} title={modal.mode === "add" ? "Add Expert" : "Edit Expert"} size="md">
          <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 no-scrollbar">
            ...
            {/* Description (replaces Expertise Area) */}
            ...
            {/* Current Problem Statement dropdown */}
            ...
            {/* Social Media Links (replaces LinkedIn URL) */}
            <div>
                  ...
                  <div key={index} style={{ display: "flex", gap: "6px", alignItems: "center" }}>
                    ...
                    {form.socialLinks.length > 1 && (
                      <button
                        ...
                        style={{ padding: "6px", background: "rgba(239,68,68,0.08)", border: "none", borderRadius: "8px", cursor: "pointer", color: "#ef4444", flexShrink: 0 }}
                      >
                        <XIcon size={14} />
                      </button>
                    )}
              ...
              <button
                ...
                style={{ marginTop: "8px", background: "rgba(255,107,0,0.08)", color: "#FF6B00", border: "none", padding: "6px 14px", borderRadius: "99px", cursor: "pointer", fontSize: "12px", fontWeight: 600 }}
              >
                + Add Another Link
              </button>
            </div>
